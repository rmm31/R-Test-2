...
numbers_report_new_boxplot <-ggplot(numbers_report_fin_Market_GDP_1_with_existing, aes(x = factor(year), y = price, color = model_existing)) + 
  #geom_violin() + 
  geom_boxplot(width = 0.3) +
  scale_y_continuous(breaks = seq(0,900, by = 25)) +
  labs(x = "Year", y = bquote('AUD/t'~CO[2]~'or /t'~CO[2]~'-e')) +
  ...
  theme(panel.grid.minor.x = element_blank()) +
  theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold")) +
  scale_color_manual(name = "Carbon value source", labels = c("Existing carbon price", "Modelled carbon value"), values = c("blue", "red", "green", "purple", "brown", "black"))
...
#Add geom_point plot with USEPA SCC values to boxplot
...
#Code for all numbers from World Bank existing carbon price (new boxplot)
